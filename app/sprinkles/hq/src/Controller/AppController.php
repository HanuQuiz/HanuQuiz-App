...
class AppController extends SimpleController
{
    ...
    {
        $app = $this->getAppFromParams($args);
        ...
        // Access-controlled page
        if (!$authorizer->checkAccess($currentUser, 'uri_app', [
                'app' => $app
            ])) {
            throw new ForbiddenException();
        }
    ...
    public function getModerators(Request $request, Response $response, $args)
    {
        ...
        $sprunje->extendQuery(function ($query) use ($app) {
            return $query->join('hq_app_moderators', function ($join) use ($app) { $join->on('hq_app_moderators.user_id', 'users.id')->where('app_id', $app->id); });
        });
     ...
     * @return App
     */
    protected function getAppFromParams($params)
    {
        // Load the request schema
